...
def process_key(key):
    ...
    with open(ROOT_DIR / "uic-data" / key_meta_name, "w") as f:
        json.dump({
            ...
            "allowed_product_owner_codes": key.allowed_product_owner_codes.product_owner_code if key.allowed_product_owner_codes and key.allowed_product_owner_codes.product_owner_code else None,
            "allowed_product_owner_name": key.allowed_product_owner_codes.product_owner_name if key.allowed_product_owner_codes and key.allowed_product_owner_codes.product_owner_name else None,
        }, f)
